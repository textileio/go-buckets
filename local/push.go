...
func (b *Bucket) PushLocal(ctx context.Context, opts ...PathOption) (roots Roots, err error) {
	...
	xr, err = b.addFiles(ctx, id, key, xr, add, args.force, args.events)
	if err != nil {
		return roots, err
	}
	if len(rm) > 0 {
		for _, c := range rm {
			var err error
			xr, err = b.rmFile(ctx, id, key, xr, c, args.force, args.events)
			if err != nil {
				return roots, err
			}
		}
...
func (b *Bucket) addFiles(
	...
	opts := []buckets.Option{buckets.WithProgress(progress)}
	if !force {
		opts = append(opts, buckets.WithFastForwardOnly(xroot))
	}
	q, err := b.c.PushPaths(ctx, id, key, opts...)
	if err != nil {
		return nil, err
	}
	defer q.Close()
...
func (b *Bucket) rmFile(
	...
	var opts []buckets.Option
	if !force {
		opts = append(opts, buckets.WithFastForwardOnly(xroot))
	}
	root, err := b.c.RemovePath(ctx, id, key, c.Path, opts...)
	if err != nil {
