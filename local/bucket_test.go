...
func TestBucket(t *testing.T) {
	...
	t.Run("Get", func(t *testing.T) {
		...
		assert.Equal(t, b.Name, "bucky")
		assert.NotEmpty(t, b.Path)
		assert.True(t, b.Thread.Defined())
...
func TestBucket_PushLocal(t *testing.T) {
	buckets := setup(t)
	buck, err := buckets.NewBucket(context.Background(), getConf(t))
	require.NoError(t, err)
...
func TestBucket_AccessRoles(t *testing.T) {
	...
	require.NoError(t, err)
	reader, err := thread.NewLibp2pPubKey(rpk).DID()
	require.NoError(t, err)
	_, wpk, err := crypto.GenerateEd25519Key(rand.Reader)
	require.NoError(t, err)
	writer, err := thread.NewLibp2pPubKey(wpk).DID()
	require.NoError(t, err)
	all, err := buck.PushPathAccessRoles(context.Background(), "file", map[did.DID]collection.Role{
		reader: collection.ReaderRole,
		writer: collection.WriterRole,
	})
	require.NoError(t, err)
	assert.Len(t, all, 2)
	assert.Equal(t, collection.ReaderRole, all[reader])
	assert.Equal(t, collection.WriterRole, all[writer])

	all, err = buck.PushPathAccessRoles(context.Background(), "file", map[did.DID]collection.Role{
		reader: collection.NoneRole,
	})
	require.NoError(t, err)
	assert.Len(t, all, 1)
	_, ok := all[reader]
