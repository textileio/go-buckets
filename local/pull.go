	...
	if len(missing) > 0 {
		...
		for _, o := range missing {
			...
			eg.Go(func() error {
				...
				return b.getFile(ctx, id, b.Key(), o, events, progress)
			})
		}
		for i := 0; i < cap(lim); i++ {
			lim <- struct{}{}
...
func (b *Bucket) listPath(
	...
	rep, err := b.c.ListPath(ctx, id, b.Key(), pth)
	if err != nil {
		return
	}
	if rep.Item.IsDir {
...
func (b *Bucket) getFile(ctx context.Context, id thread.ID, key string, o object, events chan<- Event, progress chan<- int64) error {
	...
	defer finish()
	if err := b.c.PullPath(ctx, id, key, o.path, file, buckets.WithProgress(prog)); err != nil {
		return err
	}
